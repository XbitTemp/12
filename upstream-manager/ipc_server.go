...
func (s *ManagerService) Start(tunnelName string) error {
	...
	if len(inTransition) != 0 {
		return fmt.Errorf("Please allow the tunnel Р В Р вЂ Р В РІР‚С™Р вЂ™Р’В%sР В Р вЂ Р В РІР‚С™Р Р†РІР‚С›РЎС› to finish activating", inTransition)
	}
	...
	return s.chainInstallAndArm(tunnelName, path)
}
