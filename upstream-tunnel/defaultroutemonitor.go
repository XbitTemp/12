...
func monitorDefaultRoutes(family winipcfg.AddressFamily, binder conn.BindSocketToInterface, autoMTU bool, blackholeWhenLoop bool, tun *tun.NativeTun, pinVia string) ([]winipcfg.ChangeCallback, error) {
	var minMTU uint32
	if family == windows.AF_INET {
		minMTU = 576
	} else if family == windows.AF_INET6 {
	...
	doIt := func() error {
		var err error
		if pinVia != "" {
			err = bindSocketPinned(family, binder, pinVia, &lastLUID, &lastIndex)
		} else {
