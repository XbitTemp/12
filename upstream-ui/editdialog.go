...
func newEditDialog(owner walk.Form, tunnel *manager.Tunnel) (*EditDialog, error) {
	...
	dlg.syntaxEdit.SetText(chainEditorText(tunnel, &dlg.config))
...
func (dlg *EditDialog) chainLegacyBlockToggle() {
	...
	dlg.syntaxEdit.SetText(chainKeepHop1(dlg.syntaxEdit.Text(), cfg.ToWgQuick()))
	return
...
func (dlg *EditDialog) onSaveButtonClicked() {
	...
	mainText, hopText := chainSplitEditorText(dlg.syntaxEdit.Text())
	if chainErr := chainStashHop1(hopText, newName); chainErr != nil {
		showErrorCustom(dlg, chainTitle, chainHopErrorText+chainErr.Error())
		return
	}
	cfg, err := conf.FromWgQuick(mainText, newName)
	if err != nil {
